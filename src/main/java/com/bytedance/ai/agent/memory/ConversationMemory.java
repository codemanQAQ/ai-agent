 ...
 * @param summaryMessageCount 已被摘要覆盖的历史消息数；可为空
 * @param summaryModel      生成摘要的模型 / 策略标识；可为空
...
public record ConversationMemory(
    ...
    public static ConversationMemory empty() {
        return new ConversationMemory(List.of(), Optional.empty(), null, null, List.of(), Optional.empty(), Optional.empty());
    }
    ...
    public ConversationMemory withSummary(ConversationSummary conversationSummary) {
