...
/**
 * graph turn 开始时返回的会话上下文。
 *
 * <p>对应 {@code rag_conversations} / {@code rag_conversation_messages} 的两条新增 / 既有行；
 * {@code agent_turn} 行由 graph 会话仓储维护，不在此结构里出现。
 *
 * <p>幂等检查由 graph 会话仓储在 {@code agent_turn} 上完成（命中时上层应**跳过**对
 * {@link AgentConversationSpi#beginTurn} 的调用，直接从历史 agent_turn 行恢复消息 id），
 * 因此这里**不**承担 replay 语义。
 *
 * @param conversationInternalId  rag_conversations.id（持久化层引用，graph 通常不直接用）
